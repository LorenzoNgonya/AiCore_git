# ** Hangman project
-----------------------------------------------------------------------------------
First project learning code, with the use of the following technologies: VS Code, Python, Git, Github and other minipackages.

## Milestone 1.0
-----------------------------------------------------------------------------------
- Using the *[]* python built in  list fuction functions I have created a list containing 5 of my favourites fruits and assigned the varible 'word_list' to it. By using I the print() function as demonstrated below I am able then to print my 'word_list'.
...
- I created a *while* loop with the condition set to *true* allowing the code to run continuously. In the body of the while loop I  wrote the code for the following steps instructions:
    ...
    - I then proceeded to use the an *if statement* to check if the guess is a single, alphabetical character. To check if the letter was a single character I employed the *len() function* (returns number of items in an object) and *== operator* (equal to) check if the guess was equal to a single character. To check if the letter was an alphabetical character I used the *import* statement to import *'ascii_lowercase'* from *string*. ascii_lowercase lower is a pre-initialized string used as string constant. In Python, string ascii_lowercase will return lowercase letters. I then checked if the guess was in ascii_lowercase. If both of the conditions are pass the check then,I wrote code to break out of the loop.
...
- To accomplish this I created an *if* statement to check if the guess is in the word. I employed the *in*  operator to check if the element (guess) is in the iterable (word).
    - In the body of the if statement, I created code to print a message saying "Good guess! {guess} is in the word.". For {guess} I used the .format function to show the actual guess typed by the user. 
...
- I created 2 methods:
    1. a method that will ask the user to guess a letter and that will check if the guess is in the word:
        ...
        - In the body of the method, I created code for: 
    ...
    2.  a method that will check if the guess is in the word:
        - Again I used the *def* function to create the *ask_for_input* and wrote th following code in the body:
            ...
            - I created an *if* statement that runs if the guess is NOT a single alphabetical character by implimenting the built in *ascii_lowercase* to ensure the guesss is a letter and not a number or other special charaters:
                - In the body of the if statement, I wrote code printing "Invalid letter. Please, enter a single alphabetical character."
            - I created an *elif* statement that checks if the guess is already in the list_of_guesses. This was done using in function which allowed me to pass guess through the list_of_guesses  and check if the letter has already been used by the user:
            ...
            - I then called the ask_for_input method to test your code. To do this I called *game* to instantiate the class and passed thtough wordlist as a parameter. I followed this up by calling game.ask_for_input().
...
---
- In this task I extend the check_guess_method to replace the underscores(_) in the word_guessed with the letter guesssed by the user.In the if block of your check_guess method, after the print statement:
...
- Below that *def* (define) a function called play_game with the word_list as the parameter. Inside the function. I wrote code for the following steps:
    ...
    - Below I created  a while loop and set the condition to True, in  the body I wrote code for the following:
        ...
        2. Secondly, I had to check if the 'num_letters' is greater than 0 and to accomplish this I used the *>+* equal to or greater than operator and set the value to 1. If there was 1 or more letters still in num_letters then I to called the 'ask_for_input' method for this instance so the user can continue playing the game. 
        3. Lastly, I had to check if the'num_lives' is not 0 and the 'num_letters' is not greater than 0. To achive this I employed  the *>=* equal/greater than operator to chek if the num_lives was equal to or more than 1 and the *==* equal too operator to check if the num_letters was equal to 0. If these conditions were true then the user will have won the game. However, in this step I encountered  an issue in my code as the value assigned to the num_letters was an int but some of the words had double letters which resulted in the code continously asking the user to guess a letter even when they had completed the full word or the code wouldn't print the winning message. To rectify this I updated the code in attributes of the Hangman class to change the value of num_lives to a set. This allowed me to check if the guessed letter was present in 'num_letters'. If the letter was present I employed the *.remove()* method and passed guess as a variable through it to remove the character that matched it in the set even if it was more than one letter. This then allowed me use the *len()* function with num_lives through it to use *==* operator in the if statement so it could run smoothly and have an end. I then wrote code to print a message saying 'Congratulations. You won the game!'.
